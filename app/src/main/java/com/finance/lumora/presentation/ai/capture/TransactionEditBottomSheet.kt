...
fun TransactionEditBottomSheet(
    ...
    var amountText by remember(draft.amount) {
        mutableStateOf(draft.amount.toString())
    }
    ...
    var transactionDate by remember(draft.transactionDate) {
        mutableStateOf(draft.transactionDate)
    }

    var selectedCategory by remember(resolvedDraft.category) {
        mutableStateOf(resolvedDraft.category)
    }
                ...
                Button(
                    ...
                    onClick = {
                        ...
                        if (!isValidDate(transactionDate)) {
                            dateError = "Enter a valid date in YYYY-MM-DD format."
                            return@Button
                        }
                        ...
                        val updatedDraft = resolvedDraft.copy(
                            draft = draft.copy(
                                amount = amount,
...
private fun parseIsoToMillis(isoDate: String): Long? {
    return try {
        val localDate = LocalDate.parse(isoDate.trim(), DateTimeFormatter.ISO_LOCAL_DATE)
