...
class AnalyticsViewModel @Inject constructor(
     ...
     * Refresh Trigger
     * ----------------------------------------------------
     * FIX: MutableStateFlow ignores re-assignment of an
     * equal value (no new emission), so the previous
     * "selectedFilter.value = selectedFilter.value" Refresh
     * implementation was a silent no-op. This counter is
     * combined into the pipeline purely to force a fresh
     * emission on every Refresh event, regardless of whether
     * the filter or date range actually changed.
     */
    ...
    fun onEvent(
        ...
        when (event) {
            ...
            AnalyticsEvent.Refresh -> {
                ...
                // FIX: incrementing a dedicated counter (rather
                // than re-assigning selectedFilter to itself)
                // guarantees a distinct value and therefore a
                // real re-emission through combine(), which
                // re-triggers flatMapLatest and genuinely
